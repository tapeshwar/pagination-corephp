...
//	    exit("Unable to paginate: Invalid mid_range value (must be an odd integer >= 1)");
		...
		$this->return = "<a class=\"paginate\" href=\"$this->base_url" . "?page=" . ($this->current_page - 1) . "&ipp=$this->items_per_page$this->querystring\">Previous</a>";
	} else {
	...
	for ($i = 1; $i <= $this->num_pages; $i++) {
		...
		if ((in_array($i, [1])) Or ( in_array($i, [/* ($this->num_pages - 2),($this->num_pages - 1), */$this->num_pages])) Or in_array($i, $this->range))
			$this->return .= ($i == $this->current_page And $this->items_per_page != "All") ? "<a title=\"Go to page $i of $this->num_pages\" class=\"paginatecurrent\" href=\"javascript:void(0);\">$i</a>\n" : "<a class=\"paginate\" title=\"Go to page $i of $this->num_pages\" href=\"$this->base_url?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a>\n";
		if ($this->range[$this->mid_range - 1] < $this->num_pages - 1 And $i == $this->range[$this->mid_range - 1])
			$this->return .= "<span class=\"paginate\">  ...  </span>";
		}//" . "/".($this->current_page - 1) . "?
		$this->return .= (($this->current_page < $this->num_pages And $this->total_items >= 10) And ( $this->items_per_page != "All") And $this->current_page > 0) ? "<a class=\"paginate\" href=\"$this->base_url?page=" . ($this->current_page + 1) . "&ipp=$this->items_per_page$this->querystring\">Next</a>\n" : "<span class=\"inactive\" href=\"javascript:void(0);\">Next</span>\n";
		$this->return .= ($this->items_per_page == "All") ? "<a class=\"paginatecurrent\" style=\"margin-left:10px\" href=\"javascript:void(0);\">All</a>\n" : "<a class=\"paginate\" href=\"$this->base_url?page=1&ipp=All$this->querystring\">All</a>\n";
	} else {
		for ($i = 1; $i <= $this->num_pages; $i++) {
			$this->return .= ($i == $this->current_page) ? "<a class=\"paginatecurrent\" href=\"javascript:void(0);\">$i</a> " : "<a class=\"paginate\" href=\"$this->base_url?page=$i&ipp=$this->items_per_page$this->querystring\">$i</a>";
		}
		$this->return .= "<a class=\"paginate\" href=\"$this->base_url?page=1&ipp=All$this->querystring\">All</a>\n";
	}
	$this->return = str_replace("&", "&amp;", $this->return);
	$this->limit_start = (int)($this->current_page <= 0) ? 0 : (int)($this->current_page - 1) * (int)$this->items_per_page;
...
public function display_items_per_page() {
	if (!empty($this->return)) {
	    ...
	    return "<span>Records per page : </span><select style=\"width: 100px;display: inline\" onchange=\"window.location='$this->base_url?page=1&ipp='+this[this.selectedIndex].value+'$this->querystring';return false\">$items</select>\n";
	}
}
...
public function display_jump_menu() {
	if (!empty($this->return)) {
		...
		return "<span class=\"\">Go To Page Number : </span><select style=\"width: 100px;display: inline\" onchange=\"window.location='$this->base_url?page='+this[this.selectedIndex].value+'&ipp=$this->items_per_page$this->querystring';return false\">$option</select>\n";
	}
}
